...
type RoundLevel = "Easy" | "Medium" | "Hard" | "Brutal";
...
const adminPasswordStorageKey = "moment-admin-new-password";
...
export default function MomentAdminPage() {
  ...
  async function requestAdmin<T>(
    ...
    const response = await fetch("/api/admin/new/moments", {
  ...
  async function uploadReferenceImages(draft: MomentDraft, files: FileList | null) {
    ...
    try {
      ...
      setMessage(
        selectedFiles.length === 1
          ...
          : `${selectedFiles.length} reference images added.`,
      );
    } catch (error) {
  ...
  async function removeReferenceImage(
    ...
    try {
      const payload = await requestAdmin<{ draft?: MomentDraft }>({
      ...
      setMessage(
        error instanceof Error ? error.message : "Reference removal failed.",
      );
    } finally {
      setReferenceUploadId(null);
    }
        ...
        <header className="flex flex-col justify-between gap-4 border-b border-[#d8dee9] pb-5 md:flex-row md:items-end">
          <div>
            ...
            <h1 className="font-serif text-3xl leading-tight text-[#111827]">
              New Moment Import Review
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-[#496174]">
              Paste event descriptions, attach references, generate one candidate
              with the simple 360 play prompt, then add it to the game data.
            </p>
          </div>
          ...
          <aside className="flex flex-col gap-4">
            <div className="rounded-lg border border-[#d8dee9] bg-white p-4 shadow-sm">
              ...
              <p className="mt-3 text-xs leading-5 text-[#5d7184]">
                ...
                year, sport, venue, coordinates, and per-moment prompt controls.
              </p>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {visibleDrafts.map((draft) => (
                <MomentCard
                  ...
                  onReferenceUpload={uploadReferenceImages}
...
function getReferenceImageUrls(draft: MomentDraft) {
  return Array.from(
    new Set([
...
function MomentCard({
  ...
  onReferenceUpload: (draft: MomentDraft, files: FileList | null) => Promise<void>;
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-[#d8dee9] bg-white shadow-sm">
      ...
      <div className="flex flex-col gap-3 p-4">
        <div className="grid gap-3 sm:grid-cols-[1fr_8rem]">
          <label className="flex min-w-0 flex-col gap-1">
            ...
            <input
              ...
              multiple
              onChange={(event) => {
                void onReferenceUpload(draft, event.target.files);
                event.target.value = "";
              }}
              type="file"
            />
          ...
          <div className="min-h-20 overflow-hidden rounded-md border border-[#d8dee9] bg-[#f3f6fa]">
            {referenceImageUrls.length > 0 ? (
              <div className="grid h-full min-h-20 grid-cols-3 gap-1 p-1">
                {referenceImageUrls.slice(0, 6).map((referenceImageUrl, index) => (
                  <div
                    className="relative overflow-hidden rounded bg-[#dfe6ef]"
                  ...
                  >
                    ...
                    <img
                      ...
                      src={`${referenceImageUrl}?v=${encodeURIComponent(
                        draft.updatedAt,
                      )}`}
                    />
                    {index === 5 && referenceImageUrls.length > 6 ? (
                      <span className="absolute inset-0 flex items-center justify-center bg-black/60 text-xs font-black text-white">
                        +{referenceImageUrls.length - 6}
                      </span>
                    ) : null}
                    <button
                      aria-label="Remove reference image"
                      className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-black/70 text-sm font-black leading-none text-white shadow disabled:opacity-50"
                      disabled={busy || referenceUploading}
                      onClick={() => {
                        void onReferenceRemove(draft, referenceImageUrl);
                      }}
                      type="button"
                    >
                      x
              ...
              <div className="flex h-full min-h-20 items-center justify-center px-2 text-center text-xs font-bold text-[#5d7184]">
                {referenceUploading ? "Uploading..." : "No references"}
              </div>
            )}
          </div>
        </div>
        ...
        <TextArea
          label="Custom POV"
          onChange={(customPov) => onChange(draft.id, { customPov })}
          value={draft.customPov ?? ""}
        />

        <TextArea
          label="Action Flow"
          onChange={(actionFlow) => onChange(draft.id, { actionFlow })}
          value={draft.actionFlow ?? ""}
        />

        <TextArea
          label="Sport Rules"
          onChange={(sportRules) => onChange(draft.id, { sportRules })}
          value={draft.sportRules ?? ""}
        />
            ...
            onChange={(event) =>
              onChange(draft.id, {
                level: event.target.value
                  ? (event.target.value as RoundLevel)
                  : undefined,
              })
            }
            value={draft.level ?? ""}
